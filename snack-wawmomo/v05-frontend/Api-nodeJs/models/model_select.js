...
exports.getAllProduitsOfMenus = (id) => {
    return DBconnMySql.then((conn) => {
        return conn.query("SELECT * FROM `produits` P JOIN `menus` M ON P.menu_id = M.ID WHERE P.menu_id = ? GROUP BY `nom` ORDER BY `nom` ASC", id)
    })
}
